// Сид: создаёт документы страниц «Курс (/)» и «Индивидуальные (/individual)» в Sanity, если их ещё нет.
...
const docs = [
  ...
  {
    _id: 'individualHeroBlock',
    _type: 'individualHeroBlock',
    ...
    slots: [
      { _key: key(), icon: '🗡', title: 'Одиночный рейд', sub: 'индивидуальные · 5–11 класс', href: '#formats-solo' },
      { _key: key(), icon: '🛡', title: 'Командный сектор', sub: 'мини-группы до 5 · 5–10 класс', href: '#formats-group' },
    ],
  },
  {
    _id: 'teachersBlock',
  ...
  },
  {
    ...
    benefits: [
      { _key: key(), title: 'Профессиональные', text: 'опытные преподаватели' },
      { _key: key(), title: 'Индивидуальный подход', text: 'программа под ваши цели' },
      { _key: key(), title: 'Удобный график', text: 'занимайтесь в комфортное время' },
    ],
  },
  {
    _id: 'diagnosticBlock',
    _type: 'diagnosticBlock',
    ...
    steps: [
      { _key: key(), title: 'Диагностика', text: '15 минут интерактивной симуляции определяют твои сильные стороны.' },
      { _key: key(), title: 'Выбор направления', text: 'Результат — персональная рекомендация. Финальное решение за тобой.' },
      { _key: key(), title: 'Старт подготовки', text: 'Работа с наставником, с которым реальный прогресс в подготовке.' },
    ],
  },
];
...
// ---------- Создание ----------
try {
  // Устаревший документ «Тексты страницы» (тип уже удалён из схем).
  const legacy = await client.fetch('*[_type == "homePage"]{_id}');
  if (legacy.length > 0) {
    await client.delete({ query: '*[_type == "homePage"]' });
    console.log(`удалён устаревший документ homePage (${legacy.length})`);
  }
